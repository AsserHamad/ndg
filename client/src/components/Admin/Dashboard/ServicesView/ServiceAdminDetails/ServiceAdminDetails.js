...
import './ServiceAdminDetails.css';
...
function ServiceAdminDetails(props){
    const 
        api = props.api,
        ...
        initialInputVal = {
            ...
            image: service.image,
            items_en: service.items.en || [],
            items_ar: service.items.ar || [],
        },
        ...
        useEffect(() => {
            window.scrollTo(0,0);
            let x = {};
            for(let i = 0; i < service.items.en.length; i++){
                x = ({...x, [i] : service.items.en[i]});
            }
            setItemsEn(x)
            
            x = {};
            for(let i = 0; i < service.items.ar.length; i++){
...
export default ServiceAdminDetails;